...
use self::blog_os::BlogOsFrameWriter;

pub mod blog_os;
pub mod pixel;

pub static WRITER: StaticOnceCell<StaticSpinMutex<BlogOsFrameWriter>> = StaticOnceCell::uninit();
...
pub fn init(frame_buffer: &'static mut FrameBuffer) {
    let mut writer = BlogOsFrameWriter::new(frame_buffer);
    writer.clear();
